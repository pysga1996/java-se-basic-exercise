...
public class MyLinkedList<E> {
    ...
    void add(int index, E element) {
        ...
        else if (index == 1) addFirst(element);
    ...
    E remove(int index) {
        if (index < 1 || index > numNodes) return null;
        else if (index == 1) return removeFirst();
        else if (index == numNodes) return removeLast();
        else {
    ...
    public MyLinkedList<E> clone() throws CloneNotSupportedException{
        if (numNodes == 0) throw new CloneNotSupportedException("The list is empty, you can't clone it!");
        else {
    ...
    E get(int index) {
        if (index < 1 || index > numNodes) throw new IndexOutOfBoundsException();
            Node<E> current = head;
        for (int i = 0; i < numNodes; i++) {
            if (i == index - 1) {
                break;
